...
<script type="text/javascript">
		jQuery(function($) {
			document.onkeydown = (function(e) {
				if (e.keyCode == 13) {
					if (loginForm.elements["user_id"].value == '') {
						$("#expMsg_font").html("请输入用户号");
						return false;
					}
					if (loginForm.elements["user_pass"].value == '') {
						$("#expMsg_font").html("请输入密码");
						return false;
					}
					login();
				}
		...
		function login(){
			var user_id=loginForm.elements["user_id"].value;
			var user_pass=loginForm.elements["user_pass"].value;
			$.ajax( {
				type : "POST",
				url : "<%=path%>/mvc/login.do?tradeCode=T99999&user_id="+user_id+"&user_pass="+user_pass,
				async : false,
				success : function(obj) {
					if(obj.expMsg != null){
						$("#expMsg_font").html(obj.expMsg);
					}else{
						document.location.href="<%=path%>/mvc/forwardIndex.do";
					}
				},
				error : function(msg) {
					$("#expMsg_font").html("获取数据失败: " + msg);
				}
			});
		}		
		function clearExpMsg(){
			$("#expMsg_font").html("");
		}		
	</script>
			...
			<div class="row">
				<div class="col-sm-10 col-sm-offset-1">
					<div class="login-container">
						<div class="center">
							<h1>
								<i class="icon-leaf green"></i><span class="white">五车APP管理平台</span>
							</h1>
							<!-- <h4 class="blue">&copy; 智控国际（长春）数据中心</h4> -->
						</div>
								...
								<div class="widget-body">
									...
									<!-- 	<h4 class="header blue lighter bigger">
										...
										<form method="post" id="loginForm" name="loginForm">
											<input class="hidden" type="text" name="tradeCode" data-min="2"
													data-max="20" value="T80001" />
											<fieldset>
												<label class="block clearfix"> <span
													class="block input-icon input-icon-right"> <input
														type="text" class="form-control" placeholder="用户号"
														name="user_id" id="user_id" value="" onclick="clearExpMsg()"/> <i class="icon-user"></i>
												</span>
												</label> <label class="block clearfix"> <span
													class="block input-icon input-icon-right"> <input
														type="password" class="form-control"
														placeholder="密码" name="user_pass" id="user_pass" value="000000" onclick="clearExpMsg()"/> <i
														class="icon-lock"></i>
												</span>
												<font color="red" id="expMsg_font" ></font>
												</label>
												...
												<div class="clearfix">
													...
													<button type="button"
														class="width-35 pull-right btn btn-sm btn-primary" onclick="javascript:login();">
