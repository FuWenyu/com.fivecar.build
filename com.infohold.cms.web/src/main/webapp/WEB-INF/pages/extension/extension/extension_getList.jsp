...
<html lang="en">
												...
												<c:forEach items="${extensionList}" var="bizExtensionEntity">
													<tr>
														...
														<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
															<button type="button" class="btn btn-xs btn-primary" onclick="viewEntity('${bizExtensionEntity.xh}','${bizExtensionEntity.sqzt}')">查看</button>
															<button type="button" class="btn btn-xs btn-primary" onclick="editEntity('${bizExtensionEntity.xh}','${bizExtensionEntity.sqzt}')">编辑</button>
															<button type="button" class="btn btn-xs btn-primary" onclick="cancleEntity('${bizExtensionEntity.xh}','${bizExtensionEntity.sqzt}')">取消</button>
															<button type="button" class="btn btn-xs btn-primary" onclick="startWork('${bizExtensionEntity.xh}','${bizExtensionEntity.sqzt}')">提起审批</button>
														</div>
														</td>
													</tr>
												</c:forEach>
	...
	<script type="text/javascript">
		...
		function addEntity(){
			window.location="<%=path%>/mvc/extension_add.do";
		};
		function editEntity(xh,status){
			if(status==1){
      			window.location="<%=path%>/mvc/extension_view.do?viewOrEdit=edit&xh="+xh;
			}else{
				f_alert("<%=path%>","该状态不可修改!","");			
			}
		};
		function viewEntity(xh){
			window.location="<%=path%>/mvc/extension_view.do?viewOrEdit=view&xh="+xh;
		};
		function startWork(xh,status){
			 var param={};
				...
				if(status==1){
					...
					$.post("extension_updateStatue.do?doOperate=startWork",param,function(result){
						if(result.msg=="success"){
							f_alert("<%=path%>","success","extension_getList.do?pageNo="+$("#pageNo").val());
						}else{
							f_alert("<%=path%>","提交审批失败!","");			
						}
					}); 
		...
		function cancleEntity(xh,status){
			...
			if(status==1){
				...
				$.post("extension_updateStatue.do?doOperate=cancle",param,function(result){
					if(result.msg=="success"){
						f_alert("<%=path%>","success","extension_getList.do?pageNo="+$("#pageNo").val());
					}else{
						f_alert("<%=path%>","取消失败!","");			
