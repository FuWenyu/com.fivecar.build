...
<html lang="en">
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<form class="form-horizontal" id="sub_form" action="<%=path%>/mvc/version_editSave.do"
					...
					<input id="lender_id" type="hidden" class="" name="lender_id"    value="${lender.id}" />
					<input id="resource" type="hidden" class="" name="resource" value="${lender.resourceid}-${lender.privilegestile}-${lender.privileges}" />
					<div class="form-group">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="id">
							金融机构名称  </label>
						<div class="col-sm-9">
							<input type="text" id="lenderName" name="lenderName" placeholder="请输入金融机构名称 "
								class="col-xs-10 col-sm-5"
								value="${lender.lenderName}" /> <span style="color: red"></span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="id">
							联系电话  </label>
						<div class="col-sm-9">
							<input type="text" id="telephone" name="telephone" placeholder="请输入金融机构联系电话 "
								class="col-xs-10 col-sm-5"
								value="${lender.telephone}" /> <span style="color: red"></span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="name">
							金融机构简介 </label>
						<div class="col-sm-9">
							<textarea rows="6" cols="72" id="description" name="description">${lender.description}</textarea>
						</div>
					</div>
					<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="fileInput">金融机构logo上传</label>
					...
					<div class="clearfix form-actions">
						<div class="col-md-offset-3 col-md-9">
							...
							&nbsp; &nbsp; &nbsp;
							<button class="btn" type="button" onclick="javascript:window.location = '<%=path%>/mvc/lender.do';">
	...
	function subForm() {
		if (!checkData('lenderName', '车型名称', 'input')) {
			return;
		}
		if (!checkData('lenderName', '车型名称', 'illegal')) {
			return;
		}
		if (!limitCheck('lenderName', '车型名称', 64)) {
			return;
		}
		if (!checkData('telephone', '联系电话', 'input')) {
			return;
		}
		if (!checkData('description', '金融机构简介', 'input')) {
			return;
		}
		//附件格式控制，ID，白名单
...
// 					$("#upload1").append("<label class='filelist col-sm-12'><a href=\"downloadFile.do?att_id="+obj.fileVo.attId+"\">"+obj.fileVo.attName+" &nbsp;&nbsp;&nbsp;</a><a class=\"remove\" data-attid="+obj.fileVo.attId+" data-fileName="+obj.fileVo.fileName+" href=\"#\" onclick=\"removeFile(this);\"><i class=\"icon-remove\"></i></a></label>");
	...
	function subForm1(imageName,filePath,filePathreal,imageid){
		param = {};
		param["lender_id"]=$("#lender_id").val();
		param["vehicle"]=$("#vehicle").val();
		param["lenderName"]=$("#lenderName").val();
		...
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/lender_editSave.do",
			data : param,
			async : false,
			dataType:'json',
			success : function(data) {
				if(data.msg == "success"){
					alert("保存成功！");
					window.location = "<%=path%>/mvc/lender.do";
				}else{
					alert("保存失败:"+data.msg);
				}
			},
