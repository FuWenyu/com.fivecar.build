...
%>
<!-- 模态块 -->
<div class="modal fade bs-example-modal-1g" id="duebill_Modal" tabindex="-1"
...
show_modal(1);
  ...
  function show_modal(pageNo) {
		...
		$.ajax( {
			type : "POST",
			url : "${pageContext.request.contextPath}/mvc/DueBill_queryContractList.do?pageNo="+pageNo,
			data : $("#form").serialize(),
			async : false,
			success : function(obj) {
				...
				$(contractList).each(function(index){//输出结果集到表格，此处根据各自需要修改
					var str="<tr class=\"sub_data_tr_1\" ondblclick=\"sure('"+contractList[index].contract_id+"'"+",'"+contractList[index].con_acct+"'"+",'"
							                                                 ...
							                                                 +contractList[index].rat_adj_eff_time+"'"+",'"+contractList[index].rat_exe_coe+"'"+",'"
							                                                 +contractList[index].rat_ove_cha+"'"+",'"+contractList[index].rat_ove_coe+"'"+",'"
							                                                 +contractList[index].rat_ove_val+"'"+",'"+contractList[index].rat_emb_cha+"'"+",'"
							                                                 +contractList[index].rat_emb_coe+"'"+",'"+contractList[index].rat_emb_val+"'"+",'"
							                                                 +contractList[index].rat_exe_val+"'"+",'"+contractList[index].rat_int_val+"'"+",'"
							                                                 +contractList[index].system_date+"'"+",'"+contractList[index].system_date+"')\">";
					str=str + "<td>"+contractList[index].contract_id+"</td>";
					str=str + "<td>"+contractList[index].con_acct+"</td>";
					str=str + "<td>"+contractList[index].custid+"</td>";
	...
	//模态框数据双击	
	function sure(contract_id,con_acct,amount,custid,custchname,product_name,gua_type,start_date,end_date,rat_exe_cha,rat_adj_eff_time,rat_exe_coe,
			      rat_ove_cha,rat_ove_coe,rat_ove_val,rat_emb_cha,rat_emb_coe,rat_emb_val,rat_exe_val,rat_int_val,system_date) {
		...
		document.getElementById("rat_int_val").value = rat_int_val; 
		document.getElementById("rat_exe_cha").value = rat_exe_cha; 
		document.getElementById("rat_adj_eff_time").value = rat_adj_eff_time; 
		document.getElementById("rat_exe_coe").value = rat_exe_coe; 
		document.getElementById("rat_exe_val").value = rat_exe_val; 
		document.getElementById("rat_ove_cha").value = rat_ove_cha; 
		document.getElementById("rat_ove_coe").value = rat_ove_coe; 
		document.getElementById("rat_ove_val").value = rat_ove_val; 
		document.getElementById("rat_emb_cha").value = rat_emb_cha; 
		document.getElementById("rat_emb_coe").value = rat_emb_coe; 
		...
		//借据起始日期到期日期
		document.getElementById("start_date").value = system_date; 
		document.getElementById("end_date").value = end_date;
		
		$("#duebill_Modal").modal('toggle');
