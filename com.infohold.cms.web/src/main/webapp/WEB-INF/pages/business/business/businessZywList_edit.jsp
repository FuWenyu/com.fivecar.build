...
<body id="iframe_body"	onload="setHash('${pageContext.request.contextPath}')">
									...
									<tbody>
										<c:forEach items="${fsxxList}" var="fsxxList">
											<tr>
												...
												<td>${fsxxList.xh}</td>
												<td><fmt:message key='FL_ZYW.${fsxxList.zywzl}' /></td>
												<td>${fsxxList.zywsl}</td>
												<td>${fsxxList.zywpgje}</td>
												<td>${fsxxList.zywdbje}</td>
	...
	function addEntity(businessid,bzxs,bzrsl,sqzt,custid){
		if(sqzt==8||sqzt==11){
			window.location="<%=path%>/mvc/businessFsxx_add.do?bzxs="+bzxs+"&zydbfs=4&businessid="+businessid+"&custid="+custid;
		}else{
	...
	function editEntity(xh,businessid,sqzt,custid){
		if(sqzt==8||sqzt==11){
			window.location="<%=path%>/mvc/businessFsxx_edit.do?businessid="+businessid+"&zydbfs=4&xh="+xh+"&custid="+custid;
		}else{
			f_alert("<%=path%>","当前状态不可维护!","");	
			return;
		}
	...
	function deleteEntity(xh,businessid,sqzt){
		if(sqzt==8||sqzt==11){
			var param={};
			param["xh"]=xh;
			$.post("businessFsxx_delete.do?zydbfs=4",param,function(result){
				if(result.msg=="success"){
					f_alert("<%=path%>","success","business_view.do?zydbfs=4&viewOrEdit=edit&businessid="+businessid);
				}else{
					f_alert("<%=path%>","删除失败!","");			
				}
			});
