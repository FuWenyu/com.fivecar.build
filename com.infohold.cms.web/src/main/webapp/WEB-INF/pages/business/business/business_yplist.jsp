...
%>
<!-- 模态块 -->
<div class="modal fade bs-example-modal-lg" id="ypModal" tabindex="-1"
				...
				<div class="row">
					...
					<div class="col-sm-1"></div>
						<label class="col-sm-1 control-label no-padding-right"
							for="form-field-1"> 押品名称</label>
						<div class="col-sm-3">
							<input type="text" id="ypmc"  
								class="col-xs-10 col-sm-5" name="ypmc"  value="" />
						</div>
					<div class="col-sm-1">
...
<script type="text/javascript">
function selectYpList(pageNo){
	showModal($("#ypModal"));
	var custid=document.getElementById("custid").value;
	var dywzl=document.getElementById("zl").value;
	$.ajax( {
		type : "POST",
		url : "${pageContext.request.contextPath}/mvc/business_getypList.do?pageNo="+pageNo+"&collType="+dywzl+"&custid="+custid,
		async : false,
		success : function(obj) {
				var pageInfo=obj[0];//分页信息
				var ypList=obj[1];//结果信息
				$(ypList).each(function(index){//输出结果集到表格，此处根据各自需要修改
					var $tr = "<tr class=\"sub_data_tr_2\"><td>"+ypList[index].collId+"</td> "
					+" <td>"+ypList[index].collName+"</td> "
					+" <td>"+ypList[index].evalue+"</td> "
					+" <td>"+ypList[index].value+"</td> "
					+" <td>"+ypList[index].collType+"</td> "
					+" <td>"+ypList[index].collState+"</td> "
					+" <td><button type=\"button\" onclick=\"CallBackSubmit2('"
					+ypList[index].collId+","
					+ypList[index].collName+","
					+ypList[index].evalue+","
					+ypList[index].value+","
					+ypList[index].collType+","
					+ypList[index].collState+"');\">选择</button></td> "
					+" </tr>";
...
function show_modal_query2(pageNo) {
	window.$(".sub_data_tr_2").remove();
		$.ajax( {
			...
			url : "${pageContext.request.contextPath}/mvc/business_getypList.do?pageNo="+pageNo,
			async : false,
			success : function(obj) {
				var pageInfo=obj[0];//分页信息
				var ypList=obj[1];//结果信息
...
//双击方法
function CallBackSubmit2(ypList) {
	var objs = ypList.split(",");
	$("#ypbh").val(objs[0]);
	$("#pgje").val(objs[2]);
	$("#kyye").val(objs[3]);
	$("#ypzt").val(objs[5]);
