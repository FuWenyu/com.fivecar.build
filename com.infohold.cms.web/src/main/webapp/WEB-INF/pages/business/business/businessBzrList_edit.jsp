...
<body id="iframe_body"	onload="setHash('${pageContext.request.contextPath}')">
									...
									<tbody>
										<c:forEach items="${fsxxList}" var="fsxxList">
											<tr>
												...
												<td>${fsxxList.xh}</td>
												<td>${fsxxList.bzrxm}</td>
												<td><fmt:message key='gua_form.${fsxxList.bzxs}' /></td>
												<td>${fsxxList.dbfe}</td>
												<td><div class="btn-group">
													<button class="btn btn-sm btn-primary" onclick="editEntity('${fsxxList.xh}','${bizBusinessEntity.businessid}','${bizBusinessEntity.bzxs }','${bizBusinessEntity.sqzt}','${bizBusinessEntity.custid}');" type="button">维护</button>
	...
	function addEntity(businessid,bzxs,bzrsl,sqzt,custid){
		if(sqzt==8||sqzt==11){
			if(bzxs==1&&bzrsl==1){
				f_alert("<%=path%>","保证形式为单人担保只可以存在一个保证人!","");	
				return;
			}
			window.location="<%=path%>/mvc/businessFsxx_add.do?bzxs="+bzxs+"&zydbfs=2&businessid="+businessid+"&custid="+custid;
		}else{
	...
	function editEntity(xh,businessid,bzxs,sqzt,custid){
		if(sqzt==8||sqzt==11){
			window.location="<%=path%>/mvc/businessFsxx_edit.do?bzxs="+bzxs+"&businessid="+businessid+"&zydbfs=2&xh="+xh+"&custid="+custid;
		}else{
			f_alert("<%=path%>","当前状态不可维护!","");	
			return;
		}
	...
	function deleteEntity(xh,businessid,sqzt){
		if(sqzt!=8){
			var param={};
			param["xh"]=xh;
			$.post("businessFsxx_delete.do?zydbfs=2",param,function(result){
				if(result.msg=="success"){
					f_alert("<%=path%>","success","business_view.do?zydbfs=2&viewOrEdit=edit&businessid="+businessid);
				}else{
					f_alert("<%=path%>","删除失败!","");			
				}
			}); 
