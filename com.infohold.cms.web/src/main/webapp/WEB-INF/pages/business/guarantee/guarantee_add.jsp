...
<html lang="en">
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<form id="form_sub" class="col-xs-12" action=""method="post">
							...
							<div class="span12">
								<div class="center">
										<button class="btn btn-sm btn-primary" type="button" onclick="javascript:AddOrUpdate();">保存</button>
	...
	function AddOrUpdate(){
		   ...
		   //将隐藏域的变为可见
		    document.getElementById("gua_type_List_guarantee").disabled=false;
			 	//guarantee_id如果为空就是新增，guarantee_id不为空就是修改
			   if(guarantee_id==null||guarantee_id==""){
			   	$.ajax( {
					type : "POST",
					url : "<%=path%>/mvc/guarantee_addSave.do?contract_id="+contract_id,
				   ...
				   $.ajax( {
						type : "POST",
						url : "<%=path%>/mvc/guarantee_editSave.do?contract_id="+contract_id,
						data : $("#form_sub").serialize(),
						async : false,
			   ...
			   //将隐藏域的变为可见
			    document.getElementById("gua_type_List_guarantee").disabled=false;
				   ...
				   if(guarantee_id==null||guarantee_id==""){
					   $.ajax( {
							type : "POST",
							url : "<%=path%>/mvc/guarantee_change_addSave.do?contract_id="+contract_id,
							data : $("#form_sub").serialize(),
							async : false,
