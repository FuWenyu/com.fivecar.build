...
<html lang="en">
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<form class="form-horizontal" id="sub_form" action="<%=path%>/mvc/version_editSave.do"
					...
					<input id="tpsales_id" type="hidden" class="" name="tpsales_id" value="${tpsales.id}" />
					<input id="dealer1" type="hidden" class="" name="dealer1" value="${tpsales.belong}-${tpsales.belongName}" />
					<div class="form-group">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="id">
							销售员姓名  </label>
						<div class="col-sm-9">
							<input type="text" id="tpsalesName" name="tpsalesName" placeholder="请输入销售员姓名 "
								class="col-xs-10 col-sm-5"
								value="${tpsales.salesName}" /> <span style="color: red"></span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="id">
						...
						<div class="col-sm-9">
							<input type="text" id="phone" name="phone" placeholder="请输入销售员联系电话！ "
								class="col-xs-10 col-sm-5"
								value="${tpsales.phone}" /> <span style="color: red"></span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="id">
						...
						<div class="col-sm-9">
							<input type="text" id="wxQQ" name="wxQQ" placeholder="请输入销售员微信或者QQ号码！ "
								class="col-xs-10 col-sm-5"
								value="${tpsales.wxQQ}" /> <span style="color: red"></span>
						</div>
					</div>
	...
	function subForm() {
		...
		if (!checkData('tpsalesName', '销售员姓名', 'input')) {
			return;
		}
		if (!checkData('tpsalesName', '销售员姓名', 'illegal')) {
			return;
		}
		if (!limitCheck('tpsalesName', '销售员姓名', 64)) {
			return;
		}
...
// 					$("#upload1").append("<label class='filelist col-sm-12'><a href=\"downloadFile.do?att_id="+obj.fileVo.attId+"\">"+obj.fileVo.attName+" &nbsp;&nbsp;&nbsp;</a><a class=\"remove\" data-attid="+obj.fileVo.attId+" data-fileName="+obj.fileVo.fileName+" href=\"#\" onclick=\"removeFile(this);\"><i class=\"icon-remove\"></i></a></label>");
	...
	function subForm1(imageName,filePath,filePathreal,imageid){
		param = {};
		param["tpsales_id"]=$("#tpsales_id").val();
		param["imageName"]=imageName;
		param["dealer"]=$("#dealer").val();
		param["tpsalesName"]=$("#tpsalesName").val();
		...
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/tpsales_editSave.do",
			data : param,
			async : false,
			dataType:'json',
			success : function(data) {
				if(data.msg == "success"){
					alert("保存成功！");
					window.location = "<%=path%>/mvc/tpsales.do";
				}else{
					alert("保存失败:"+data.msg);
				}
			},
