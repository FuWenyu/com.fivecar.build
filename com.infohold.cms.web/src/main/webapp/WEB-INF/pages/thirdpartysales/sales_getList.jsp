...
<html lang="en">
	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<!-- 查询条件begin -->
				<form class="col-xs-12" id="queryForm" name="qry_form" action="tpsales.do?type=cms" method="post">
					...
					<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample-table-2">
						...
						<tbody>
							<c:forEach items="${tpsalesList}" var="tpsalesList">
								<tr>
									<%-- javascript:viewFile('${tpsalesList.from_url }','${tpsalesList.file_name}') --%>
									<td>
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("tpsales.do", param, function(result) {			
		...
		//跳转至图片新增页面
		function addEntity(){
			window.location="<%=path%>/mvc/tpsales_add.do";
		};
		//跳转至图片编辑页面
		function editEntity(id){
			window.location="<%=path%>/mvc/tpsales_edit.do?id="+id+"";
		};
		
		//删除图片By id
		function deleteEntity(tpsalesid){
			//异步删除 成功后跳转页面
			var param={};
			param["id"]=tpsalesid;
			$.post("<%=path%>/mvc/tpsales_delete.do",param,function(result){
				movePage(1);
			});
		};
	</script>
