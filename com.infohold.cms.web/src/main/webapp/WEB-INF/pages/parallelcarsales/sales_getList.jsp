...
<html lang="en">
	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<!-- 查询条件begin -->
				<form class="col-xs-12" id="queryForm" name="qry_form" action="pasales.do?type=cms" method="post">
					...
					<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample-table-2">
						...
						<tbody>
							<c:forEach items="${salesList}" var="salesList">
								<tr>
									<%-- javascript:viewFile('${salesList.from_url }','${salesList.file_name}') --%>
									<td>
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("pasales.do", param, function(result) {			
		...
		function editEntity(id){
			window.location="<%=path%>/mvc/pasales_edit.do?id="+id+"";
		};
		
		//删除图片By id
		function deleteEntity(salesid){
			//异步删除 成功后跳转页面
			var param={};
			param["id"]=salesid;
			$.post("<%=path%>/mvc/pasales_delete.do",param,function(result){
				movePage(1);
			});
		};
	</script>
