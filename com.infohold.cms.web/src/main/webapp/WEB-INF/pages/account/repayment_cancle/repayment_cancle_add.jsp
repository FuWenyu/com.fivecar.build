...
<html lang="en">
		...
		<div class="page-content" id="loadPageContent">
			<div class="row">
				<div class="col-xs-12">
					...
					<h3 class="header smaller lighter blue">出账信息</h3>
					<div class="form-group">
						<label class="col-sm-2 control-label no-padding-right"
							for="duebill_id"> 出账编号 </label>
						<div class="col-sm-3">
							<input name="flow_no" id="flow_no" placeholder="点击选择" type="text"
					...
					<div class="form-group">
						...
						<label class="col-sm-2 control-label no-padding-right" for="amount">
							还款金额 </label>
						<div class="col-sm-5">
							<input name="repay_amount" id="repay_amount" class="col-xs-10 col-sm-5" type="text" readonly>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label no-padding-right"
				       ...
				       <div class="form-group">
						...
						<div class="col-sm-3">
							<input name="branchName" id="branchName" 
								   class="col-xs-10 col-sm-5" type="text" value="${sessionScope.userSession.branchName}" readonly>
						</div>
						<label class="col-sm-2 control-label no-padding-right" for="userName">
							经办客户经理 </label>
						<div class="col-sm-5">
							<input name="userName" id="userName" class="col-xs-10 col-sm-5" type="text" value="${sessionScope.userSession.userName}" readonly>
						</div>
					</div>
					<div class="form-group">
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">出账列表</h4>
				</div>
	...
	function subForm() {
		...
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/repmentAcc_cancle_addSave.do",
			...
			success : function(data) {
				if(data.msg == "success"){
					alert("保存成功！");
					window.location = "<%=path%>/mvc/repmentAcc_cancle_getList.do";
				}else{
					alert("保存失败！");
				}
			},
	...
	function selectContract(){
		...
		$.ajax( {
			type : "POST",
			url : "${pageContext.request.contextPath}/mvc/repmentAcc_cancle__queryDuebill.do",
			async : false,
			success : function(obj) {
				$(obj).each(function(index){
					var $tr = "<tr><td>"+$(obj)[index].flow_no+"</td> "
								+" <td>"+$(obj)[index].duebill_acct+"</td> "
								+" <td>"+$(obj)[index].custid+"</td> "
								+" <td>"+$(obj)[index].cust_name+"</td> "
								+" <td>"+$(obj)[index].repay_amount+"</td> "
								...
								+" <td><button type=\"button\" onclick=\"CallBackSubmit('"
										...
										+$(obj)[index].end_date+","
										+$(obj)[index].re_id+","
										+$(obj)[index].repay_amount+"');\">选择</button></td> "
								+" </tr>";
