...
<html lang="en">
	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="sub_form" action="<%=path%>/mvc/pabrand_editSave.do"
					method="post">
				...
				<div class="form-group">
					...
					<div class="clearfix form-actions">
						<div class="col-md-offset-3 col-md-9">
							...
							&nbsp; &nbsp; &nbsp;
							<button class="btn" type="button" onclick="javascript:window.location = '<%=path%>/mvc/pabrand.do';">
...
// 					$("#upload1").append("<label class='filelist col-sm-12'><a href=\"downloadFile.do?att_id="+obj.fileVo.attId+"\">"+obj.fileVo.attName+" &nbsp;&nbsp;&nbsp;</a><a class=\"remove\" data-attid="+obj.fileVo.attId+" data-fileName="+obj.fileVo.fileName+" href=\"#\" onclick=\"removeFile(this);\"><i class=\"icon-remove\"></i></a></label>");
	...
	function subForm1(imageName,filePath,filePathreal,imageid){
		...
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/pabrand_editSave.do",
			data : param,
			async : false,
			dataType:'json',
			success : function(data) {
				if(data.msg == "success"){
					alert("保存成功！");
					window.location = "<%=path%>/mvc/pabrand.do";
				}else{
					alert("保存失败:"+data.msg);
				}
			},
