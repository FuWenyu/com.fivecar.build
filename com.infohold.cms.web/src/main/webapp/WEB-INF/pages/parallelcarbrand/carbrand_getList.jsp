...
<html lang="en">
	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<!-- 查询条件begin -->
				<form class="col-xs-12" id="queryForm" name="qry_form" action="pabrand.do?type=cms" method="post">
					...
					<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample-table-2">
						...
						<tbody>
							<c:forEach items="${brandList}" var="brandList">
								<tr>
									<%-- javascript:viewFile('${brandList.from_url }','${brandList.file_name}') --%>
									<td>
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("pabrand.do", param, function(result) {			
		...
		//跳转至图片新增页面
		function addEntity(){
			window.location="<%=path%>/mvc/pabrand_add.do";
		};
		//跳转至图片编辑页面
		...
		function deleteEntity(brandid){
			...
			var param={};
			param["id"]=brandid;
			$.post("<%=path%>/mvc/pabrand_delete.do",param,function(result){
				movePage(1);
			});
		};
	</script>
