...
<html lang="en">
	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<!-- 查询条件begin -->
				<form class="col-xs-12" id="queryForm" name="qry_form" action="resources.do?type=cms" method="post">
					<input class="hidden" type="text" name="tradeCode" id="tradeCode" data-min="2" data-max="20" value="T10010" />
					<div class="row">
						<div class="space-6"></div>
						<div class="form-group">
							<div class="col-md-3"></div>
							<label class="col-sm-1 control-label no-padding-right" for="form-field-2">资源名称</label>
							<div class="col-sm-3">
								<input id="createName" name="createName" type="text" >
							</div>
				...
				<div id="sample-table-2_wrapper" class="dataTables_wrapper" role="grid">
					<table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample-table-2">
						...
						<tbody>
							<c:forEach items="${resourcesList}" var="resourcesList">
								<tr>
									<td>${resourcesList.resourceName}</td>
									<td>${resourcesList.title}</td>
									<td>${resourcesList.createDate}</td>
									<td>${resourcesList.createName}</td>
									<td>
										<!-- 列表按钮区域 -->
					...
					<div class="row">
						<div class="col-sm-4">
							<div class="btn-group">
								<!-- 列表底部按钮区域 -->
							 	<button class="btn btn-sm btn-primary" onclick="addResourcesEntity();"
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("resources.do", param, function(result) {			
		...
		//跳转至图片新增页面
		function addResourcesEntity(){
			window.location="<%=path%>/resources_add.do";
		};
		//跳转至图片编辑页面
		function editEntity(id){
			window.location="<%=path%>/resources_edit.do?id="+id+"";
		};
		...
		function deleteEntity(id){
			...
			var param={};
			param["id"]=id;
			$.post("<%=path%>/mvc/resources_delete.do",param,function(result){
				movePage(1);
			});
		};
	</script>
