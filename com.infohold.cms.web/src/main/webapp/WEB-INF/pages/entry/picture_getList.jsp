	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<!-- 查询条件begin -->
				<form class="col-xs-12" id="queryForm" name="qry_form" action="entry.do?usefo=${usfo}" method="post">
					<input class="hidden" type="text" name="usfo" id="usfo" value="${usfo}" />
					<div class="row">
...
<%-- 			window.open("<%=path%>/upload/imagereal/"+from_url+"/"+file_name); --%>
		...
		function addPictureEntity(){
			var usefo = $("#usfo").val();
			window.location="<%=path%>/mvc/entry_add.do?usefo="+usefo+"";
		};
		//跳转至图片编辑页面
