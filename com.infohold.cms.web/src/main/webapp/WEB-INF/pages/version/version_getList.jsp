	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<!-- 查询条件begin -->
				<form class="col-xs-12" id="queryForm" name="qry_form" action="version.do" method="post">
					<input class="hidden" type="text" name="tradeCode" id="tradeCode" data-min="2" data-max="20" value="T10010" />
<!-- 					<input class="hidden" type="text" name="pageNo" id="pageNo" data-min="2" data-max="20" value=${page.pageNo} /> -->
					<div class="row">
						<div class="space-6"></div>
						<div class="form-group">
							<div class="col-md-3"></div>
							<label class="col-sm-1 control-label no-padding-right" for="form-field-2">版本号</label>
							<div class="col-sm-3">
						...
						<tbody>
							<c:forEach items="${versionList}" var="versionList">
								<tr>
									...
									<%-- <td>
										<c:if test="${versionList.status == '0'}">离线</c:if>
										<c:if test="${versionList.status == '1'}">在线</c:if>
									</td> --%>
									<td>
										<!-- 列表按钮区域 -->
...
<!-- 											<button type="button" class="btn btn-xs btn-primary" onclick="addversionEntity()">新增</button> -->
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("version_getList.do", param, function(result) {			
		...
		//跳转至用户新增页面
		function addversionEntity(){
			window.location="<%=path%>/mvc/version_add.do";
		};
		//跳转至用户编辑页面
		function editEntity(id){
			window.location="<%=path%>/mvc/version_edit.do?id="+id+"";
		};
		//跳转至用户查看页面
		...
		//删除用户By id
		function deleteEntity(version_id){
			//异步删除 成功后跳转页面
			var param={};
			param["version_id"]=version_id;
			$.post("<%=path%>/mvc/version_delete.do",param,function(result){
				movePage(1);
			});
		};
	</script>
