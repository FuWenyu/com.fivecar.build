...
<html lang="en">
	...
	<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
		...
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="sub_form" action="<%=path%>/mvc/version_addSave.do"
					method="post">
					<h3 class="header smaller lighter grey">版本新增</h3>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="text" id=version_desc placeholder="请输入版本描述"
								class="col-xs-10 col-sm-5" name="version_desc" value="" />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="text" id="version_url" placeholder="请输入版本地址"
								class="col-xs-10 col-sm-5" name="version_url" value="" />
						</div>
					</div>
...
// 			$("#cms_form-group").find("input[value=2]").attr("checked","checked").attr("disabled","disabled");
		...
		function subForm() {
			...
			$.ajax({
				type : "POST",
				url : "<%=path%>/mvc/version_addSave.do",
