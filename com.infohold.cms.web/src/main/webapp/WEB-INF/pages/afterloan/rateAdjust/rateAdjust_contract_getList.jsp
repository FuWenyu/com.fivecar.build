...
%>
		...
		<div class="row">
			<div class="col-xs-12">
				...
				action="<%=path%>/mvc/rateAdjust_contract_getList.do" method="post">
	...
	<script type="text/javascript">
		...
		function addEntity(){
			$.post("rateAdjust_contract_add.do", param, function(result) {			
		...
		function editEntity(rate_id){
			...
			$.post("rateAdjust_contract_edit.do", param, function(result) {			
				$("#tabContent").html(result).hide();
				$("#tabContent").fadeIn('fast');
				setHash('${pageContext.request.contextPath}');
			});
		...
		function viewEntity(rate_id){
			param={};
			param["rate_id"] = rate_id;
			$.post("rateAdjust_contract_view.do", param, function(result) {			
				$("#tabContent").html(result).hide();
				$("#tabContent").fadeIn('fast');
				setHash('${pageContext.request.contextPath}');
			});
		...
		function CallBackSubmit(obj){
			...
			$.ajax({
    			type : "POST",
    			url : "<%=path%>/mvc/rateAdjust_contract_approve.do",
    			data : param,
    			async : false,
    			success : function(data) {
    				if(data.msg == "success"){
