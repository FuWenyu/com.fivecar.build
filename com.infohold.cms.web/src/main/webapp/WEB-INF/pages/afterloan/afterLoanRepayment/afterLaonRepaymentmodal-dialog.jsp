...
function selectDuebill(pageNo){
	...
	$.ajax( {
		type : "POST",
		url : "${pageContext.request.contextPath}/mvc/afterLoanRepayment_getModal.do?pageNo="+pageNo,
		async : false,
		success : function(obj) {
				var pageInfo=obj[0];//分页信息
				var BizDueBillList=obj[1];//结果信息
				$(BizDueBillList).each(function(index){//输出结果集到表格，此处根据各自需要修改
					var $tr = "<tr class=\"sub_data_tr_1\"><td>"+BizDueBillList[index].duebill_acct+"</td> "
					+" <td>"+BizDueBillList[index].con_acct+"</td> "
					+" <td>"+BizDueBillList[index].custid+"</td> "
					+" <td>"+BizDueBillList[index].custchname+"</td> "
					...
					+BizDueBillList[index].status_name+","
					+BizDueBillList[index].billstart+","
					+BizDueBillList[index].custchname+","
					+BizDueBillList[index].name_+","
					+BizDueBillList[index].billend+"');\">选择</button></td> "
					+" </tr>";
					$(window.document).find("#data").append($tr);
				});
				//输出分页信息，下面的不需要动				
...
function show_modal_query(pageNo) {
	window.$(".sub_data_tr_1").remove();
		$.ajax( {
			type : "POST",
			data : $(window.document).find("#myModal").serialize(),
			url : "${pageContext.request.contextPath}/mvc/afterLoanRepayment_getModal.do?pageNo="+pageNo,
			async : false,
			success : function(obj) {
				var pageInfo=obj[0];//分页信息
...
function CallBackSubmit(businessList) {
	...
	document.getElementById("cust_id").value = objs[4];
	document.getElementById("agreement_start").value = objs[5];
	document.getElementById("agreement_end").value = objs[6];
	
	document.getElementById("duebill_id_2").value = objs[7];
	document.getElementById("iou_amount").value = objs[8];
	document.getElementById("rat_int_val").value = objs[9];
	document.getElementById("rat_exe_val").value = objs[10];
	document.getElementById("rat_ove_val").value = objs[11];
	
	document.getElementById("rat_emb_val").value = objs[12];
	document.getElementById("loan_acct_no").value = objs[13];
	document.getElementById("iou_status").value = objs[14];
	document.getElementById("iou_start").value = objs[15];
	
	document.getElementById("cust_name").value = objs[16];
	document.getElementById("repayment_type").value = objs[17];
	
	document.getElementById("iou_end").value = objs[18];
	...
	//调核心还款账户信息查询，返回页面需要信息
	 $.ajax({
			type : "POST",
			url : "<%=path%>/mvc/afterLoanRepayment_Invoke470009.do?duebill_id="+duebill_id,
			async : false,
			success : function(obj) {
				//document.getElementById("repay_amount").value = obj.arrears_all_amt;
				document.getElementById("accrued_amt").value = obj.accrued_amt;
				document.getElementById("arrears_amt").value = obj.arrears_amt;
				document.getElementById("arrears_int_amt").value = obj.arrears_int_amt;
				document.getElementById("punitive_int_amt").value = obj.punitive_int_amt;
				document.getElementById("surplus_amt").value = obj.surplus_amt;
				document.getElementById("arrears_all_amt").value = obj.arrears_all_amt;
			},
			error : function(msg) {
