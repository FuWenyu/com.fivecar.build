...
<body id="iframe_body"
	...
	<div class="page-content" id="loadPageContent">
		<div class="row">
			<div class="col-xs-12">
				...
				action="<%=path%>/mvc/loanReport_getList.do" method="post">
				<div class="form-group">
					<div class="col-md-1"></div>
					<label class="col-sm-1 control-label no-padding-right"
						for="report_id"> 报告编号</label>
					<div class="col-sm-4">
						<input type="text" id="report_id" class="col-xs-10 col-sm-5"
							name="report_id" />
					</div>
					<label class="col-sm-1 control-label no-padding-right"
						for="status"> 报告状态</label>
					<div class="col-sm-4">
						<select class="col-xs-10 col-sm-5" id="status" name="status">
							<option value="">请选择...</option>
							<option value="0">已提交</option>
							<option value="1">发起审批</option>
							...
							<tr>
								...
								<td>
									<c:if test="${rsList.status  == '1' || rsList.status  == '6'}">
										<button class="btn btn-sm btn-primary" onclick="editEntity('${rsList.report_id}');"
											type="button">修改</button>
										<button class="btn btn-sm btn-primary" onclick="approveEntity('${rsList.report_id}');"
											type="button">提起审批</button>
									</c:if>
									<button class="btn btn-sm btn-primary" onclick="viewEntity('${rsList.report_id}');"
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("loanReport_getList.do", param, function(result) {			
		...
		function addEntity(){
			window.location="<%=path%>/mvc/loanReport_add.do";
		};
		function editEntity(report_id){
			window.location="<%=path%>/mvc/loanReport_edit.do?report_id="+report_id;
		};
		function viewEntity(report_id){
			window.location="<%=path%>/mvc/loanReport_view.do?report_id="+report_id;
		};
		function approveEntity(report_id){		
			showModal($("#myModal"));
			$(window.document).find("#report_id").val(report_id);
				
		};
		function CallBackSubmit(obj){
			var param={};
			param["report_id"]=$(obj).closest("#myModal").find("#report_id").val();
			param["reason_approve"]=$(obj).closest("#myModal").find("#reason_approve").val();
			$.ajax({
    			type : "POST",
    			url : "<%=path%>/mvc/loanReport_approve.do",
    			data : param,
    			async : false,
