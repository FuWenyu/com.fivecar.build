...
<body id="iframe_body"
		...
		jQuery(function($) {
			...
			$('#fuelux-wizard').ace_wizard(
				...
				if($("#contributor_cardno").val()!=null &&$("#contributor_cardno").val()!="" ){
					...
					if($("#contributor_cardtype").val()=="01" || $("#contributor_cardtype").val()=="01"){
	...
	function subForm(){
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/corCustomer_editSave_temp.do",
			data : $("#sub_form").serialize(),
			async : false,
			dataType:'json',
			success : function(data) {
				if(data.msg == "success"){
					f_alert("<%=path%>","success","corCustomer_getList_temp.do?pageNo="+$("#pageNo").val());
				}else{
