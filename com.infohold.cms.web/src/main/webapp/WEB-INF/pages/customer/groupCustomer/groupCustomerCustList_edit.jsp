...
<body id="iframe_body"	onload="setHash('${pageContext.request.contextPath}')">
	...
	<div class="page-content" id="loadPageContent">
		...
		<input type="hidden" id="groupid" name="groupid"	value="${groupCustomerEntity.groupid }" />
										...
										<c:forEach items="${groupCustListList}" var="groupCustListList">
											<tr>
												...
												<td><div class="btn-group">
													<button class="btn btn-sm btn-primary" onclick="editEntity('${groupCustListList.id}');" type="button">维护</button>
													<button class="btn btn-sm btn-primary" onclick="deleteEntity('${groupCustListList.id}');" type="button">删除</button>
												</div></td>
											</tr>
										</c:forEach>
									</tbody>
								...
								<div id="sample-table-2_wrapper" class="dataTables_wrapper" role="grid">
									<div class="row">
										<div class="col-sm-4">
											<div class="btn-group">
												<!-- 列表底部按钮区域 -->
												<button class="btn btn-sm btn-primary" onclick="addEntity('${groupCustomerEntity.groupid }');" type="button">新增</button>
											</div>
										</div>
								    </div>
							    </div>
	...
	function addEntity(id){
		window.location="<%=path%>/mvc/groupCustomerMember_add.do?addOrEdit=add&groupid="+id;
	}
	function editEntity(id){
		window.location="<%=path%>/mvc/groupCustomerMember_add.do?addOrEdit=edit&id="+id;
	}
	function deleteEntity(id){
		 var param={};
		param["id"]=id;
		$.post("groupCustomerMember_delete.do",param,function(result){
			if(result.msg=="success"){
				f_alert("<%=path%>","success","groupCustList_get.do?viewOrEdit=edit&id="+$("#groupid").val());
			}else{
				f_alert("<%=path%>","删除失败!","");			
			}
		}); 
