...
%>
<!-- 模态块 -->
<div class="modal fade bs-example-modal-lg" id="Member_Modal" tabindex="-1"
...
<script type="text/javascript">
	...
	<script type="text/javascript">
function selectCustomer(pageNo,groupid){
	showModal($("#Member_Modal"));
	$.ajax( {
		type : "POST",
		url : "${pageContext.request.contextPath}/mvc/group_getCustomerList.do?pageNo="+pageNo+"&is_member=1"+"&groupid="+groupid,
		async : false,
			...
			$(obj[1]).each(function(index){
				 var $tr = "<tr class=\"sub_data_tr_1\"><td>"+$(obj[1])[index].cust_id+"</td> "
							...
							+" <td><button type=\"button\" onclick=\"CallBackSubmit('"
									...
									+$(obj[1])[index].file_type+","
									+$(obj[1])[index].file_number+","
...
function show_modal_query(pageNo) {
	window.$(".sub_data_tr_1").remove();
		$.ajax( {
			...
			url : "${pageContext.request.contextPath}/mvc/group_getCustomerList.do?pageNo="+pageNo+"&is_member=1",
			async : false,
			success : function(obj) {
				var pageInfo=obj[0];//分页信息
				var custList=obj[1];//结果信息
...
function CallBackSubmit(custList){
	...
	$("#file_type").val(objs[2]);
	$("#file_number").val(objs[3]);
	$("#organizationcode").val(objs[4]);
	hideModal();
}
</script>
