...
<body id="iframe_body"	onload="setHash('${pageContext.request.contextPath}')">
	...
	<div class="page-content" id="loadPageContent">
						...
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								<select id="group_relationship" class="form-control"  name="group_relationship" 
									data-select-value="1" data-select-list="${group_relationshipList }" >
								</select>
							</div>
						</div>
					</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								<select id="group_custtype" class="form-control"  name="group_custtype" 
									data-select-value="1" data-select-list="${group_custtypeList }" >
								</select>
							</div>
						</div>
					</div>
				...
				<div class="row">
						...
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								<select id="is_mycust" class="form-control"  name="is_mycust" onchange="ismycust()"
									data-select-value="1" data-select-list="${yesornoList }" disabled="disabled">
								</select>
							</div>
						</div>
					</div>
					<div class="form-group col-sm-6">
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								<input type="text" id="cust_id" name="cust_id" onclick="selectCustomer(1,${groupCustListEntity.groupid})"
				...
				<div class="row">
					<div class="form-group col-sm-6">
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								<select id="file_type" class="form-control"  name="file_type" data-emptyoption="true"
									data-select-value="" data-select-list="${cor_id_typeList }" disabled="disabled">
								</select>
							</div>
						</div>
					</div>
		...
		<!-- 引模态 -->
 <jsp:include page="getGroupMember_modal.jsp"></jsp:include><!-- 引客户 -->
	...
	function subForm(){
		...
		/* if ($("#group_relationship").val()=="-1") {
			$("#group_relationship").closest("form").find(".help-block").remove();
			$("#group_relationship").after("<div class=\"help-block red\" >&nbsp;&nbsp;*&nbsp;[集团关联关系类型]为必输项！");
			$("#group_relationship").focus();
			return false;
		}
		if ($("#group_custtype").val()=="-1") {
			$("#group_custtype").closest("form").find(".help-block").remove();
			$("#group_custtype").after("<div class=\"help-block red\" >&nbsp;&nbsp;*&nbsp;[集团成员类型]为必输项！");
			$("#group_custtype").focus();
			return false;
		} */
		...
		/* if ($("#file_type").val()=="-1") {
			$("#file_type").closest("form").find(".help-block").remove();
			$("#file_type").after("<div class=\"help-block red\" >&nbsp;&nbsp;*&nbsp;[文件类型]为必输项！");
			$("#file_type").focus();
			return false;
		} */
		...
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/groupCustomerMember_addSave.do",
