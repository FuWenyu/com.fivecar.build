...
<body id="iframe_body"
	...
	<div class="page-content" id="loadPageContent">
		<div class="row">
			<div class="col-xs-12">
				...
				action="<%=path%>/mvc/customerMove_getList.do" method="post">
				<div class="form-group">
					<div class="col-md-1"></div>
					<label class="col-sm-1 control-label no-padding-right"
						for="cust_id"> 客户编号编号</label>
					<div class="col-sm-4">
						<input type="text" id="cust_id" class="col-xs-10 col-sm-5"
							name="cust_id" />
					</div>
					<label class="col-sm-1 control-label no-padding-right"
						for="status"> 状态</label>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-xs-12 center">
							<input type="hidden" id="move_id" name="move_id">
	...
	<script type="text/javascript">
		...
		function toQry()
		{
			...
			param += "&qry_type=qry";
			$.post("customerMove_getList.do", param, function(result) {			
		...
		function addEntity(){
			window.location="<%=path%>/mvc/customerMove_add.do";
		};
		function editEntity(move_id){
			window.location="<%=path%>/mvc/customerMove_edit.do?move_id="+move_id;
		};
		function viewEntity(move_id){
			window.location="<%=path%>/mvc/customerMove_view.do?move_id="+move_id;
		};
		function approveEntity(move_id){		
			showModal($("#myModal"));
			$(window.document).find("#move_id").val(move_id);
				
		};
		function CallBackSubmit(obj){
			var param={};
			param["move_id"]=$(obj).closest("#myModal").find("#move_id").val();
			param["reason_approve"]=$(obj).closest("#myModal").find("#reason_approve").val();
			$.ajax({
    			type : "POST",
    			url : "<%=path%>/mvc/customerMove_approve.do",
    			data : param,
    			async : false,
