...
<body id="iframe_body" onload="setHash('${pageContext.request.contextPath}')">
	...
	<div class="row">
	<div class="col-xs-12">
		<form class="col-xs-12" id="queryForm" name="queryForm" action="<%=path%>/mvc/perCustomer_getList.do" method="post">
			<div class="row">
								...
								<tbody>
									<c:forEach items="${perCustomerList}" var="perCustomerList">
										<tr>
											...
											<c:if test="${perCustomerList.cust_status ==4}">
												<td class="tooltip-info" title="${perCustomerList.cust_replyexp}" data-rel="tooltip" data-placement="left"><fmt:message key='cust_status.${perCustomerList.cust_status}' /></td>
											</c:if>
											<td>
												<div class="btn-group">
	...
	jQuery(function($){
		...
		function viewEntity(id){
			window.location="<%=path%>/mvc/customer_tab.do?id="+id+"&obj=1&pageNo="+$("#pageNo").val();
		}
		function editEntity(id){
			window.location="<%=path%>/mvc/perCustomer_get.do?id="+id+"&viewOrEdit=edit&pageNo="+$("#pageNo").val();
		}
		function querySubmit(){
			var param = $("#queryForm").serialize();
			param += "&qry_type=qry";
			$.post("perCustomer_getList.do", param, function(result) {			
