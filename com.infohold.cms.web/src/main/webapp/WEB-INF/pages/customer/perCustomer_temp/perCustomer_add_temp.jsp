...
<body id="iframe_body"
					...
					<div class="form-group col-sm-6">
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								...
								class="form-control" placeholder="证件号码">
							</div>
						</div>
					</div>
				</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-6 input-group">
								<input type="text" id="work_startdate"  name="work_startdate" value="${perCustomerEntity.work_startdate }" data-date-format="yyyy-mm-dd"
									class="form-control date-picker"  placeholder="本单位工作起始年份">
								<span class="input-group-addon"> 
									<i class="icon-calendar bigger-110"></i>
								</span>
							</div>
	...
	function subForm(){
		$.ajax({
			type : "POST",
			url : "<%=path%>/mvc/perCustomer_addSave_temp.do",
			data : $("#sub_form").serialize(),
			async : false,
			dataType:'json',
			success : function(data) {
				if(data.msg == "success"){
					f_alert("<%=path%>","success","perCustomer_getList_temp.do");
				}else{
